...
export const fetch = (slug, useCache = false) => ({
  type: ARTICLE_PAGE_LOAD,
  payload: Promise.all([
    API.Articles.get({ slug, requestId: 'ARTICLE_PAGE_LOAD', config: { cache: useCache } }),
    API.Comments.get({ slug, requestId: 'ARTICLE_PAGE_LOAD', config: { cache: useCache } }),
  ])
})
